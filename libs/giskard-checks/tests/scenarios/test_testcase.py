...
class TestTestCaseNormalCases:
    ...
    async def test_testcase_with_single_passing_check(self):
        ...
        check = Equality(expected="test_output", key="trace.interactions[-1].outputs")
        test_case = TestCase(
            name="single_check",
            trace=trace,
            checks=[check],
    ...
    async def test_testcase_with_multiple_passing_checks(self):
        ...
        checks = [
            Equality(expected=42, key="trace.interactions[-1].outputs.result"),
            Equality(expected="ok", key="trace.interactions[-1].outputs.status"),
        ]
        test_case = TestCase(
    ...
    async def test_testcase_with_failing_check(self):
        ...
        check = Equality(expected="expected", key="trace.interactions[-1].outputs")
        test_case = TestCase(
            name="failing_check",
            trace=trace,
            checks=[check],
    ...
    async def test_testcase_with_multiple_checks_one_fails(self):
        ...
        checks = [
            Equality(expected="output", key="trace.interactions[-1].outputs"),  # Passes
            Equality(expected="wrong", key="trace.interactions[-1].outputs"),  # Fails
            Equality(expected="output", key="trace.interactions[-1].outputs"),  # Passes
        ]
        test_case = TestCase(
            name="mixed_results",
    ...
    async def test_testcase_with_multiple_failures_all_run(self):
        ...
        checks = [
            Equality(expected="output", key="trace.interactions[-1].outputs"),  # Passes
            Equality(expected="wrong1", key="trace.interactions[-1].outputs"),  # Fails
            Equality(expected="wrong2", key="trace.interactions[-1].outputs"),  # Fails
            Equality(expected="output", key="trace.interactions[-1].outputs"),  # Passes
        ]
        test_case = TestCase(
    ...
    async def test_testcase_without_name(self):
        ...
        check = Equality(expected="output", key="trace.interactions[-1].outputs")
        test_case = TestCase(
            name=None,
            trace=trace,
            checks=[check],
...
class TestTestCaseResult:
    ...
    async def test_testcase_result_assert_passed_failure(self):
        ...
        check = Equality(expected="wrong", key="trace.interactions[-1].outputs")
        test_case = TestCase(
...
class TestTestCaseEdgeCases:
    ...
    async def test_testcase_with_complex_interaction(self):
        ...
        check = Equality(
            expected="Hi Alice!", key="trace.interactions[-1].outputs.response"
        )
        test_case = TestCase(
    ...
    async def test_testcase_with_dynamic_outputs(self):
        ...
        check = Equality(
            expected="Processed: test", key="trace.interactions[-1].outputs"
        )
        test_case = TestCase(
