...
class InteractionRecord[InputType, OutputType](BaseModel, frozen=True):
    """An immutable record of a single exchange in a chat or workflow.

    Unlike `Interaction` (which dynamically generates inputs at runtime),
    a `FrozenInteraction` is fully static and predictable. It captures
    the concrete inputs, outputs, and optional metadata as they occurred.
    Used within a `Trace` to represent the materialized history of a scenario.
