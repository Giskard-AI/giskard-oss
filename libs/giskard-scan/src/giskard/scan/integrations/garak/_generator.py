...
def _await_on_loop[T](
    ...
    Garak calls ``_call_model`` synchronously, often from ``asyncio.to_thread``
    workers. ``asyncio.run`` there creates ephemeral loops that break
    ``asyncio.Lock`` in ``DatasetInputGenerator`` when probes run in parallel.
    When *loop* is the scan's running loop, schedule the coroutine there via
