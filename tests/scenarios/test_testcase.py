...
class TestTestCaseNormalCases:
    ...
    async def test_testcase_with_single_passing_check(self):
        ...
        check = EqualityCheck(expected="test_output", key="interactions[-1].outputs")
        test_case = TestCase(
            name="single_check",
            trace=trace,
            checks=[check],
    ...
    async def test_testcase_with_multiple_passing_checks(self):
        ...
        checks = [
            EqualityCheck(expected=42, key="interactions[-1].outputs.result"),
            EqualityCheck(expected="ok", key="interactions[-1].outputs.status"),
        ]
        test_case = TestCase(
    ...
    async def test_testcase_with_failing_check(self):
        ...
        check = EqualityCheck(expected="expected", key="interactions[-1].outputs")
        test_case = TestCase(
            name="failing_check",
            trace=trace,
            checks=[check],
    ...
    async def test_testcase_with_multiple_checks_one_fails(self):
        ...
        checks = [
            EqualityCheck(expected="output", key="interactions[-1].outputs"),  # Passes
            EqualityCheck(expected="wrong", key="interactions[-1].outputs"),  # Fails
            EqualityCheck(expected="output", key="interactions[-1].outputs"),  # Passes
        ]
        test_case = TestCase(
            name="mixed_results",
    ...
    async def test_testcase_with_multiple_failures_all_run(self):
        ...
        checks = [
            EqualityCheck(expected="output", key="interactions[-1].outputs"),  # Passes
            EqualityCheck(expected="wrong1", key="interactions[-1].outputs"),  # Fails
            EqualityCheck(expected="wrong2", key="interactions[-1].outputs"),  # Fails
            EqualityCheck(expected="output", key="interactions[-1].outputs"),  # Passes
        ]
        test_case = TestCase(
    ...
    async def test_testcase_without_name(self):
        ...
        check = EqualityCheck(expected="output", key="interactions[-1].outputs")
        test_case = TestCase(
            name=None,
            trace=trace,
            checks=[check],
...
class TestTestCaseResult:
    ...
    async def test_testcase_result_assert_passed_failure(self):
        ...
        check = EqualityCheck(expected="wrong", key="interactions[-1].outputs")
        test_case = TestCase(
...
class TestTestCaseEdgeCases:
    ...
    async def test_testcase_with_complex_interaction(self):
        ...
        check = EqualityCheck(
            expected="Hi Alice!", key="interactions[-1].outputs.response"
        )
        test_case = TestCase(
    ...
    async def test_testcase_with_dynamic_outputs(self):
        ...
        check = EqualityCheck(
            expected="Processed: test", key="interactions[-1].outputs"
        )
        test_case = TestCase(
